...
export function ElasticEmailCSVImport() {
  ...
  const parseCSV = async (file: File) => {
    try {
      ...
      const emailCol = headers.findIndex(h => 
        h.toLowerCase().includes('email') || h.toLowerCase() === 'email'
      );
  ...
  const uploadToElasticEmail = async () => {
    ...
    try {
      ...
      toast.success(`Successfully uploaded ${csvData.length} contacts to Elastic Email!`);
      ...
      toast.error(`Failed to upload contacts: ${error.message || 'Unknown error'}`);
    } finally {
          ...
          <div className="space-y-4">
            <div className="space-y-2">
              ...
              <p className="text-sm text-muted-foreground">
                CSV should contain columns: email (required), firstName, lastName
              </p>
            </div>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm"><strong>{csvData.length}</strong> contacts found</span>
              </div>
              <div className="flex items-center gap-2">
