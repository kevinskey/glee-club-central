...
export const TourMerchAssignment: React.FC<TourMerchAssignmentProps> = ({
  ...
  const calculateSuggestedQuantity = (item: StoreItem): number => {
    ...
    if (item.tags.includes('CD') || item.tags.includes('music')) {
      ...
      affinityFactor = 0.6;
    } else if (item.tags.includes('clothing') || item.tags.includes('apparel')) {
      affinityFactor = 0.3;
    }
  ...
  const getTotalAssigned = () => {
    return Object.values(editingQuantities).reduce((sum, qty) => sum + (qty || 0), 0);
  };
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            ...
            <div className="bg-green-50 p-4 rounded-lg">
              ...
              <div className="text-sm text-green-600">Total Assignment Value</div>
            </div>
            <div className="bg-yellow-50 p-4 rounded-lg">
              <div className="text-2xl font-bold text-yellow-600">
                {storeItems.filter(item => (editingQuantities[item.id] || 0) > 0).length}
              </div>
              <div className="text-sm text-yellow-600">Items with Assignments</div>
            </div>
          </div>
            ...
            <div className="mt-4 p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
              <div className="flex items-center gap-2 text-yellow-800">
                ...
                <span className="text-sm">
                  You have read-only access. Only Tour Managers, Merchandise Managers, Treasurers, and Admins can edit assignments.
                </span>
              </div>
            </div>
          )}
