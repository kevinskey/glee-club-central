          ...
          <CardContent className="space-y-4">
            <div>
              ...
              <Select 
                ...
                onValueChange={(value) => setNewSchedule(prev => ({ ...prev, playlist_id: value }))}
              >
                <SelectTrigger>
      ...
      <div className="grid gap-4">
        {scheduledPlaylists.length === 0 ? (
          ...
          scheduledPlaylists.map((schedule) => (
            <Card key={schedule.id} className={isCurrentlyActive(schedule) ? 'border-green-500 bg-green-50 dark:bg-green-950/20' : ''}>
              <CardContent className="p-6">
                <div className="flex justify-between items-start">
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-2">
                      <h4 className="font-semibold">{schedule.playlists.name}</h4>
                    ...
                    <div className="flex items-center gap-4 text-sm text-muted-foreground">
                      <div className="flex items-center gap-1">
                        ...
                        Starts: {format(new Date(schedule.start_time), 'MMM d, yyyy')}
                      </div>
                      <div className="flex items-center gap-1">
                      ...
                      {schedule.end_time && (
                        <div>
                          Ends: {format(new Date(schedule.end_time), 'MMM d, yyyy h:mm a')}
                        </div>
                      )}
                    </div>
                  </div>
