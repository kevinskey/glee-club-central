    ...
    <div className="w-full">
      <div className="flex justify-between items-center mb-4">
        <div className="flex items-center gap-2">
          <Calendar className="h-4 w-4 md:h-5 md:w-5 text-glee-purple" />
          <h2 className="text-lg md:text-2xl font-semibold">Upcoming Events</h2>
        </div>
        <Link to="/calendar">
          <Button variant="ghost" size="sm" className="text-glee-purple hover:text-glee-purple/90 text-xs md:text-sm">
        ...
        <Card className="overflow-hidden border-dashed border-2 bg-muted/30">
          <CardContent className="p-4 md:p-8 flex flex-col items-center justify-center text-center">
            <div className="w-12 h-12 md:w-16 md:h-16 rounded-full bg-muted flex items-center justify-center mb-4">
              <CalendarIcon className="h-6 w-6 md:h-8 md:w-8 text-muted-foreground" />
            </div>
            <h3 className="text-lg md:text-xl font-semibold mb-1 md:mb-2">No Upcoming Events</h3>
            <p className="text-muted-foreground mb-4 md:mb-6 text-sm md:text-base max-w-md">
              Stay tuned! The Spelman College Glee Club will announce their next performances soon.
            </p>
            <Link to="/calendar">
              <Button variant="outline" size={isMobile ? "sm" : "default"} className="border-glee-purple text-glee-purple hover:bg-glee-purple hover:text-white">
                Check Full Calendar
              </Button>
            </Link>
          </CardContent>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
          {events.map((event) => (
            <Card key={event.id} className="overflow-hidden hover:shadow-md transition-shadow duration-200">
              <div className={`${isMobile ? 'h-24' : 'h-36'} bg-muted relative`}>
                {event.image ? (
                  <img
                    src={event.image}
                    alt={event.title}
                  ...
                  <div className="w-full h-full bg-gradient-to-r from-glee-purple/20 to-glee-purple/10 flex items-center justify-center">
                    <CalendarIcon className="h-10 w-10 text-glee-purple/40" />
                  </div>
                )}
              ...
              <CardContent className="p-3 md:p-4">
                <div className="space-y-2 md:space-y-3">
                  <div className="flex items-start gap-2">
                    <CalendarIcon className="h-3.5 w-3.5 md:h-4 md:w-4 text-glee-purple mt-0.5 flex-shrink-0" />
                    <span className="font-medium text-sm md:text-base">{event.date}</span>
                  </div>
                  <div className="flex items-start gap-2">
                    <Clock className="h-3.5 w-3.5 md:h-4 md:w-4 text-glee-purple mt-0.5 flex-shrink-0" />
                    <span className="text-muted-foreground text-sm md:text-base">{event.location}</span>
                  </div>
                  {!isMobile && (
                  ...
                  <Link to={`/calendar?event=${event.id}`}>
                    <Button 
                      ...
                      className="w-full mt-1 md:mt-2 border-glee-purple text-glee-purple hover:bg-glee-purple hover:text-white text-sm"
                    >
                      View Details
                    </Button>
                  </Link>
