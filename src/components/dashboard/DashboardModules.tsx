...
import { 
  ...
  Upload, Map, Layout, Heart, Bell, 
  BarChart, Search, Calendar, Users, Settings
} from 'lucide-react';
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
      {availableModules.map((module) => (
        ...
        >
          <div className={`${module.color || 'bg-primary'} text-white p-3 rounded-lg w-12 h-12 flex items-center justify-center mb-3`}>
            {iconMap[module.icon] || <FileText className="h-6 w-6" />}
          </div>
          <h3 className="text-lg font-semibold mb-1">{module.title}</h3>
          <p className="text-muted-foreground text-sm flex-1">{module.description}</p>
